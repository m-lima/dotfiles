...
function mksshid {
  ...
  if [ -f "${base}/modules/shared/services/ssh/_secrets/${host}.age" ]; then
    echo -n "[33mWARNING!![m There already exists an SSH key at '${base}/modules/shared/services/ssh/_secrets/${host}.age'. Proceed? [y/N/a] "
    read input
    case "${input}" in
      [Yy])
        rm "${base}/modules/shared/services/ssh/_secrets/${host}.age"
        rm "${base}/modules/shared/services/ssh/_secrets/${host}.pub"
        ;;
      [Aa]) exit ;;
      *) return ;;
    esac
  ...
  temp="$(mktemp)" &&
    ...
    mv "${temp}.pub" "${base}/modules/shared/services/ssh/_secrets/${host}.pub" &&
    cd "${base}" &&
    nix run github:oddlama/agenix-rekey -- edit -i "${temp}" "./modules/shared/services/ssh/_secrets/${host}.age" &&
    rm "${temp}"

  git add "./modules/shared/services/ssh/_secrets/${host}.age"
  git add "./modules/shared/services/ssh/_secrets/${host}.pub"
}
